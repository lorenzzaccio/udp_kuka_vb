   ...
   Dim NrOfEntries As Integer
    ...
    'To get local IP address
     ...
     ipCombo.Clear
      ...
      Dim i As Integer
   For i = 0 To NrOfEntries
    ipCombo.AddItem (IpAddrs(i))
   Next
   ipCombo.ListIndex = i - 1
...
Private Sub udpPeerA_DataArrival(ByVal bytesTotal As Long)
    ...
    If (bStart = 23) And (bEnd = 32) Then
        ...
        bLength = Mid(strData, 5, 2)
        k = 0
        While k < bLength
            iData = Mid(strData, 7 + k, 2)
            k = k + 1
        Wend
        szFunc = bFunc
        szLength = bLength
        If bLength > 0 Then
            szArg = Mid(result, 10, 2 * bLength + bLength)
        Else
            szArg = ""
        End If
        ' send a ack
    ...
    Case SET_REG_CMD:
        If length = 4 Then
           ...
           End If
            ...
            'MsgBox ("Connection=" & ret & "address=" & "$OUT[" & szAddr & "]")
        End If
    Case GET_REG_CMD:
        If length = 2 Then
            address = szArg(0) & szArg(1)
            ...
            result = 0
            End If
            sendBuff = "23" & GET_REG_CMD & "04" & Format(address, "0000") & Format(result, "0000") & "32"
             'MsgBox (sendBuff)
            bSendVar = True
        End If
